...
**核心交互范式**：[[playbooks/chat-pattern|Chat 对话式]] + [[playbooks/generative-interface|生成式界面]]
...
[[playbooks/focus-on-key-feature|关键功能聚焦]]：只做一件事——你的私人资料 + AI 对话。不做笔记编辑器（Notion 做），不做搜索引擎（Perplexity 做），不做知识管理（Obsidian 做）。

[[playbooks/adapt-to-new-tech|技术适配]]：核心是 Google 自家 Gemini 1.5 Pro 的长上下文能力。1M token 上下文窗口让"整本书进去"成为标准功能，这是 OpenAI 短期内的技术劣势。
...
**[[playbooks/urgent-demand|迫切刚需]]**：研究生写论文、律师查案例、分析师读财报——这些用户不需要被教育，他们只需要知道这个工具存在。

---

### 阶段五：增长引擎

**[[playbooks/product-as-content|病毒传播-产品即内容]]**：Audio Overview 本身就是可分享的内容——用户在社交媒体上分享 AI 生成的播客片段，每一次分享都是产品展示。这是 NotebookLM 增长最重要的引擎。

**[[playbooks/ride-the-wave|借势崛起]]**：Google 的分发网络（Search、Gmail、Drive、Classroom）是隐形武器。NotebookLM 与 Google Workspace 的打通让企业用户直接从 Drive 上传文档。
...
**[[playbooks/source-lockin|源材料锁定]]**：用户上传的所有资料都在 NotebookLM 里——迁移成本不是技术成本，而是"重新整理所有 Notebook"的心理成本。

**[[playbooks/brand-trust|品牌信任]]**：在"AI 幻觉"成为公众认知的时代，"基于你的资料"是极强的信任锚点。专业用户对准确性的要求使得 NotebookLM 的差异化壁垒反而因 AI 的普及而加强。

**[[playbooks/ai-as-moat|AI就是壁垒]]**：Gemini 长上下文是护城河的技术基础——1M token 上下文让竞争对手用 GPT-4 很难复刻相同体验。
